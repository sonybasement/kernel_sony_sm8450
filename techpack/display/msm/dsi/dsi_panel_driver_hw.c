...
int dsi_panel_driver_adjust_brightness_type2(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	switch (panel->spec_pdata->hmd_mode) {
		case HMD_MODE1:
			if (bl_lvl <= 278)
				return 4 + (bl_lvl - 0) * (326 - 4) / (278 - 0);

			if (bl_lvl <= 1016)
				return 613 + (bl_lvl - 278) * (2047 - 613) / (1016 - 278);

			if (bl_lvl <= 2658)
				return 2048 + (bl_lvl - 1016) * (bl.bl_max_level - 2048) / (2658 - 1016);
		...
		case HMD_OFF:
			if (bl_lvl <= 1716)
				return 4 + (bl_lvl - 0) * (2047 - 4) / (1716 - 0);

			return 2048 + (bl_lvl - 1716) * (bl.bl_max_level - 2048) / (bl.brightness_max_level - 1716);
		default:
			return dsi_panel_driver_adjust_brightness_default(panel, bl_lvl);
	}
}
